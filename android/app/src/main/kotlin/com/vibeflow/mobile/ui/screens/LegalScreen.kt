...
/** Renders a bundled policy with a tiny markdown-ish formatter (# / ## / - / blank lines). */
@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun LegalScreen(doc: LegalDoc, modifier: Modifier = Modifier, onBack: () -> Unit) {
    val content = if (doc == LegalDoc.Privacy) LegalText.PRIVACY else LegalText.TERMS
    Scaffold(
        modifier = modifier,
        topBar = {
            TopAppBar(
                title = { Text(if (doc == LegalDoc.Privacy) "Privacy Policy" else "Terms of Service", fontWeight = FontWeight.Bold) },
        ...
        Column(
            ...
            content.trimIndent().lines().forEach { line ->
                when {
                    line.startsWith("# ") -> {
                    ...
                    else -> Text(line, style = MaterialTheme.typography.bodyMedium, color = MaterialTheme.colorScheme.onSurfaceVariant)
                }
            }
 ...
 */
object LegalText {
    const val CONTACT = "vamsy.24@gmail.com"
