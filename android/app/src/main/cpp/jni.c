...
Java_com_whispercpp_whisper_WhisperLib_00024Companion_fullTranscribe(
    ...
    params.print_realtime = false;      // printing each segment in realtime wastes cycles
    params.print_progress = false;
    params.print_timestamps = false;    // we don't use word timings — skip the work
    ...
    // SPEED (biggest win): encode only as much mel context as the audio actually needs. Whisper
    // pads every clip to 30 s (1500 mel frames) and the encoder cost scales ~quadratically with
    // that context — so a 4 s dictation otherwise pays the SAME encoder cost as 30 s. 16 kHz audio
    // → ~50 frames/s after the encoder's 2x downsample, so frames ≈ samples/320 (+a small margin).
    // Floor at 512 so very short clips keep enough positional context to stay accurate.
    int audio_ctx = (int) (audio_data_length / 320) + 64;
    ...
    double audio_s = audio_data_length / 16000.0;
    clock_gettime(CLOCK_MONOTONIC, &t_start);
    LOGI("About to run whisper_full (audio=%.1fs audio_ctx=%d threads=%d)", audio_s, audio_ctx, num_threads);
    int rc = whisper_full(context, params, audio_data_arr, audio_data_length);
    clock_gettime(CLOCK_MONOTONIC, &t_end);
    if (rc != 0) {
        LOGI("Failed to run the model");
    } else {
        double took_ms = (t_end.tv_sec - t_start.tv_sec) * 1000.0 + (t_end.tv_nsec - t_start.tv_nsec) / 1e6;
        // rtf < 1.0 means faster-than-realtime — the threshold for chunked streaming to be worth it.
        LOGI("whisper_full done: took=%.0fms rtf=%.2f", took_ms, audio_s > 0 ? (took_ms / 1000.0) / audio_s : 0.0);
    }
